...
log using Data_Analysis_sensitivity5M, replace

*Use annual per capita debt data set
use annual_debt_percapita_sensitivity5M.dta
...
outreg2 using results_sensitivity5M.doc, label ctitle(OLS)
...
outreg2 using results_sensitivity5M.doc, append label ctitle(IPW, No Controls)

reg dd_debt all_shale_growth pcearn_2003 pop_growth_pct [pweight=w], robust
outreg2 using results_sensitivity5M.doc, append label ctitle(IPW, Earnings & Pop. Growth)
...
outreg2 using results_sensitivity5M.doc, append label ctitle(IPW, All Controls)

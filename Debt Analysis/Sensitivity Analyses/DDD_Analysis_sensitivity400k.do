...
log using Data_Analysis_sensitivity400k, replace

*Use annual per capita debt data set
use annual_debt_percapita_sensitivity400k.dta
...
outreg2 using results_sensitivity400k.doc, label ctitle(OLS)

*** IPW models with robust standard errors

*** Generate predicated treatment group probabilitiesl
...
outreg2 using results_sensitivity400k.doc, append label ctitle(IPW, No Controls)

reg dd_debt all_shale_growth pcearn_2003 pop_growth_pct [pweight=w], robust
outreg2 using results_sensitivity400k.doc, append label ctitle(IPW, Earnings & Pop. Growth)
...
outreg2 using results_sensitivity400k.doc, append label ctitle(IPW, All Controls)

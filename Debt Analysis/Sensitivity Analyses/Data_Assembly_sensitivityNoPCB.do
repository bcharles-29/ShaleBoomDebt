...
*** NOTE: File is identical to main analysis version, except it uses a list of counties that incurred any 
*** Pollution Control Bond (PCB) debt and drops those counties from the analysis. The list of PCB counties
*** is generated from a data file of individual bond issuances, which cannot be public posted 
*** without risking violation of the database license. The PCB list file is generated from the data file
*** of individual debt issuances

local DEBT "../Main Analysis"
local GROUPS "../../Community Group Identification/Main Analysis"
...
log using Data_Assembly_sensitivityNoPCB, replace
...
use Community_Groups_1.dta

merge m:m GEOID using Community_Groups_2.dta
drop _merge
...
save annual_debt_percapita_sensitivityNoPCB.dta, replace
...
*** Drop counties with pollution control bond debt/pop_2003
merge m:1 GEOID using pcb_list.dta
drop _merge
replace pcb=0 if missing(pcb)
drop if pcb==1
drop pcb
